...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState<Stats>({ totalBookmarks: 0, unreadBookmarks: 0, totalGenres: 0 })
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [genresRes, bookmarksRes, statsRes] = await Promise.all([
          fetch('/api/genres'),
        ...
        if (bookmarksRes.ok) setRecentBookmarks(await bookmarksRes.json())
